...
class InpTrainer(nn.Module):
    ...
    #         save_path = './checkpointsfornanjing_XN/Unet_'  # + '_%s' % timestr
    ...
    def save_net(self, save_path=None, **kwargs):
        if save_path is None:
            base_path = './checkpoints4train/Network_'
            counter = 1  # 初始化计数器
            while True:
                save_path = base_path + str(counter) + ".pth"
                if not os.path.exists(save_path):  # 检查文件是否已存在
                    break
                counter += 1  # 文件已存在，递增计数器
            torch.save(self.net.state_dict(), save_path)
        return save_path
    ...
    #                 new_state_dict[key[7:]] = value  # 移除 "module."
    #             else:
...
#         loss = loss1  
#         # + 0.0008 * loss2+ 0*loss3 + 0*l1_norm
...
#         if save_path is None:
#             timestr = time.strftime('%m%d%H%M')
#             save_path = './checkpointsfornanjing_line600/Unet_'# + '_%s' % timestr
#             # save_path = f'./checkpointsfornanjing_line600/Unet_{timestr}.pth'
#             for k_, v_ in kwargs.items():
#                 # save_path += f'
#             # for k_, v_ in kwargs.items():
#                 save_path += '_%s' % v_
